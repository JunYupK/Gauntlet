...
하네스(§5~§12)가 만든 정적 번들을 스펙 §9.2 순서의 여섯 화면으로
그리는 Next.js 정적 export다. 백엔드도 네트워크도 없이 발표 당일이
돌아가야 한다(R4) — API 라우트도 서버 액션도 두지 않는다
(`next.config.ts`의 `output: 'export'`).

**`ARENA_BUNDLE`엔 기본값이 없다.** `web/src/lib/bundle.ts`가 이 값을
못 읽으면 빌드를 그 자리에서 죽인다 — 발표용 빌드가 값을 빠뜨린 채
조용히 데모 번들로 나가는 사고를 막기 위해서다. 두 값의 뜻:

| 명령 | `ARENA_BUNDLE` | 무엇을 읽나 |
|---|---|---|
| `npm run build` | `public/data` | **진짜 번들**(`./gradlew record`가 만든 것). 발표용 |
| `npm run build:demo` | `fixtures/data` | **데모 번들**(`./gradlew fixture`가 만든 것, 커밋됨). `DataSourceBanner`가 주황 띠로 "데모 번들"임을 화면 맨 위에 계속 띄운다 |

발표 직전에 `npm run build`가 아니라 `npm run build:demo`로 나간 export를
그대로 올리면 그 띠가 청중 앞에서 그대로 보인다 — 조용히 작은 글씨가
아니라 일부러 크고 눈에 띄게 만든 것이다(`DataSourceBanner.tsx`).

**청/주황은 좌석 색이다(봇0 = 청 `#38bdf8`, 봇1 = 주황 `#fb923c`,
`lib/colors.ts`).** 색각 이상에서도 구분되고 프로젝터 대비에 강하다는
것이 채택 사유이므로, 화면 2(개선 곡선)·화면 6(히트맵)처럼 좌석과 무관한
지표에 다른 뜻의 색을 새로 써야 할 때도 이 청/주황을 재사용하지 않는다
— 재사용하면 "봇 0/1"과 "그 화면만의 지표"가 같은 색으로 겹쳐 뜻이
흐려진다.

**리플레이 디코더(`web/src/lib/replay.ts`)는 엔진 규칙의 사본이다.**
`Replay.moves` 문자열을 격자 상태로 되돌리는 이 화면 쪽 로직은 엔진의
이동 판정을 그대로 다시 구현한 것이라, 엔진이 그 규칙을 바꾸면
디코더도 조용히 틀려질 수 있다. 그래서 이 디코더를 고칠 때는 **엔진과
대조하는 conformance 테스트**(`web/src/test/replay.conformance.test.ts`)를
...
§9.2 — 결과 먼저, 과정 나중. 바꾸지 않는다). `Deck`이 모든 화면에
마운트되어 ←/→로 화면을 넘기고 Space로 재생/정지 버튼을 토글한다.

**스모크(`web/e2e/smoke.spec.ts`, Playwright)는 시각 회귀 테스트가
아니다**(스펙 §14가 금지한다). 정적 export를 실제 브라우저로 열어
콘솔 오류 0건과 필수 요소(갤러리 패널 수, 재생 시 턴 카운터 변화)만
본다. `@playwright/test`는 캐럿 없이 `1.56.0`으로 정확히 고정돼 있다 —
이유는 `web/playwright.config.ts` 상단 주석에 적혀 있다(설치된 Chromium
리비전과의 짝을 깨지 않기 위해서다).

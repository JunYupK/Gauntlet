...
주기 때문). static·synthetic 필드는 건너뛴다. `static final` 상수는 허용,
인스턴스 필드는 어느 조상 클래스에 있어도 반려다.

**G3 금지 API.** 바이트코드(ASM) 스캔이다. 봇의 최상위 클래스뿐 아니라
**중첩 클래스·익명 클래스·로컬 클래스까지 재귀적으로 따라간다** — 금지 호출을
헬퍼 클래스로 옮겨도 피할 수 없다.
...
**G4 합법 수.** 심사 시드로 생성한 10,000개 표본 국면(`GateRunner.SAMPLE_SIZE`)
전부에서 예외를 던지지 않고 null을 반환하지 않아야 한다. "죽지 않는 수"를
요구하는 게 아니다 — 자멸은 봇의 자유이며 지표로만 남는다.
...
`gate`를 통과해야 `challenge`를 돌릴 수 있다. `challenge`는 심사 시드
1‥50에서 챔피언과 교대 좌석으로 붙는다. **승점 승률이 60% 이상**

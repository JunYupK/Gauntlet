...
/**
 * 강한 함정 봇 — 순전히 실력만을 위한 픽스처다. G7의 accept 경로를
 * 실제로 확인하려면 "베이스라인을 이길 만큼 강한 봇"이 있어야 하는데,
 * {@link CleanBot}은 그 일을 하도록 만들어진 게 아니다(D51 참고).
 *
 * CleanBot과 역할이 다르다 — CleanBot은 "관문이 통과시켜야 할 것까지
 * 반려하지 않는가"(G2~G6, 계약 준수)를 확인하는 대조군이라 짧고
 * 뻔해야 값어치가 있다. 이 봇은 반대로 "관문이 실제로 강한 봇을
 * 통과시키는가"(G7, 실력)를 확인하는 게 유일한 목적이라, 그 목적에
 * 필요한 만큼 복잡해도 된다 — 오히려 복잡할수록 제 몫을 한다.
 *
 * 아래 이력은 원래 CleanBot으로 개발되던 과정을 그대로 옮긴 것이다
 * (D50). WallAvoidBot 상대 100판(judgingSeeds 1..50, 좌석 교대 포함)
 * 기준 패배 수:
 *
 *   1) 1칸 앞 빈 칸 수                                       48패
 ...
 * 직접 재생해 보니, StrongBot이 상대와 나란히(내가 y=29행, 상대가 바로
 ...
 * 탐색)이 필요해 보인다.
 */
public final class StrongBot implements Bot {
    ...
    @Override
    public String name() { return "StrongBot"; }
